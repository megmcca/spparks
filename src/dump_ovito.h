...
DumpStyle(ovito,DumpOvito)

#else

#ifndef SPK_DUMP_OVITO_H
#define SPK_DUMP_OVITO_H

#include "dump.h"

namespace SPPARKS_NS {

class DumpOvito : public Dump {
 public:
  DumpOvito(class SPPARKS *, int, char **);
  virtual ~DumpOvito();
 ...
 protected:
  ...
  typedef void (DumpOvito::*FnPtrHeader)(bigint, double);
  FnPtrHeader header_choice;           // ptr to write header functions
  void header_binary(bigint, double);
  void header_text(bigint, double);

  typedef void (DumpOvito::*FnPtrData)(int, double *);
  FnPtrData write_choice;              // ptr to write data functions
  void write_binary(int, double *);
  void write_text(int, double *);

  typedef void (DumpOvito::*FnPtrPack)(int);
...
E: Invalid attribute in dump ovito command
...
E: Region ID for dump ovito does not exist
