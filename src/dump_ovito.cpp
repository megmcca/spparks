...
enum{ID,STYPE,SITE,X,Y,Z,ENERGY,PROPENSITY,IARRAY,DARRAY};  // in other dump files
...
DumpOvito::DumpOvito(SPPARKS *spk, int narg, char **arg) : Dump(spk, narg, arg)
{
...
void DumpOvito::init_style()
{
  ...
  int flag = 0;
  for (int i = 0; i < size_one; i++)
    if (pack_choice[i] == &DumpOvito::pack_propensity) flag = 1;
  ...
  if (iregion >= 0) {
    ...
    if (iregion == -1) 
      error->all(FLERR,"Region ID for dump ovito does not exist");
  }
...
void DumpOvito::pack(tagint *ids)
{
...
void DumpOvito::write_header(bigint ndump, double time)
{
...
void DumpOvito::write_data(int n, double *buf)
{
...
void DumpOvito::header_binary(bigint ndump, double time)
{
...
void DumpOvito::header_text(bigint ndump, double time)
{
...
void DumpOvito::write_binary(int n, double *buf)
{
  n *= size_one;
...
void DumpOvito::write_text(int n, double *buf)
{
  int i,j;
...
int DumpOvito::parse_fields(int narg, char **arg)
{
  int i;
  for (int iarg = 4; iarg < narg; iarg++) {
    i = iarg-4;
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      fields[i] = ID;
      pack_choice[i] = &DumpOvito::pack_id;
      vtype[i] = TAGINT;
    } else if (strcmp(arg[iarg],"type") == 0) {
      fields[i] = STYPE;
      pack_choice[i] = &DumpOvito::pack_stype;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"site") == 0) {
      fields[i] = SITE;
      pack_choice[i] = &DumpOvito::pack_site;
      vtype[i] = INT;
      if (app->iarray == NULL)
	error->all(FLERR,"Dumping a quantity application does not support");
    } else if (strcmp(arg[iarg],"x") == 0) {
      fields[i] = X;
      pack_choice[i] = &DumpOvito::pack_x;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"y") == 0) {
      fields[i] = Y;
      pack_choice[i] = &DumpOvito::pack_y;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"z") == 0) {
      fields[i] = Z;
      pack_choice[i] = &DumpOvito::pack_z;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"energy") == 0) {
      fields[i] = ENERGY;
      pack_choice[i] = &DumpOvito::pack_energy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"propensity") == 0) {
      fields[i] = PROPENSITY;
      pack_choice[i] = &DumpOvito::pack_propensity;
      vtype[i] = DOUBLE;
    ...
    // double value = dN
      ...
      fields[i] = IARRAY;
      pack_choice[i] = &DumpOvito::pack_iarray;
      vtype[i] = INT;
	...
	error->all(FLERR,"Invalid keyword in dump command");
      ...
      fields[i] = DARRAY;
      pack_choice[i] = &DumpOvito::pack_darray;
      vtype[i] = DOUBLE;
      vindex[i] = atoi(&arg[iarg][1]);
...
int DumpOvito::modify_param(int narg, char **arg)
{
...
void DumpOvito::pack_site(int n)
{
  int *site = app->iarray[0];
...
void DumpOvito::pack_energy(int n)
{
  if (latticeflag) {
    for (int i = 0; i < nchoose; i++) {
...
void DumpOvito::pack_propensity(int n)
{
  if (latticeflag) {
    for (int i = 0; i < nchoose; i++) {
...
void DumpOvito::pack_iarray(int n)
{
...
void DumpOvito::pack_darray(int n)
{
